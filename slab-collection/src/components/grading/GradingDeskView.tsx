 ...
 * best-case first. This is the batch view where the per-card confidence bars become real odds —
 * graded as a group, wins at these bars pay for the losses.
 *
 * Same two contracts as the per-card panel: the bar framing (never "X% of the time" for one
 * card), and metric meaning rendered from the response's `grading.*` glossary.
 */
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 max-w-2xl text-sm text-slate-400" title={g("ten_confidence_needed")?.detail}>
            {sweep
              ? `${sweep.deskable} of your ${sweep.total_raw_copies} raw copies have a graded market to read. The bar is how sure you'd need to be a card gems before grading beats keeping it raw — you can see its corners; slab can't.`
              : "Which of your raw cards are worth grading?"}
          </p>
        </div>
      ...
      {sweep && sweep.entries.length === 0 && !loading ? (
        <div className="rounded-2xl border border-slate-800 bg-slate-900/40 p-6 text-sm text-slate-400">
          None of your raw cards have graded sales to compare against yet, so there are no
          verdicts to give. That&apos;s the honest answer, not an error — coverage grows as slab
          keeps collecting graded sales.
        </div>
      ) : null}
        ...
        <div className={`overflow-x-auto rounded-2xl border border-slate-800 transition-opacity ${loading ? "opacity-50" : ""}`}>
          <table className="min-w-full text-left text-sm">
            <thead className="bg-slate-950/60 text-[11px] uppercase tracking-wide text-slate-500">
              <tr>
                <th className="px-4 py-3 font-medium" title={g("verdict")?.detail}>Verdict</th>
                ...
                <th className="px-4 py-3 font-medium" title={g("ten_confidence_needed")?.detail}>Bar</th>
...
const VERDICT_BADGE: Record<GradingVerdict, { text: string; className: string }> = {
  easy_yes: { text: "Send it", className: "border-emerald-500/30 bg-emerald-500/10 text-emerald-300" },
  judgment_call: { text: "Your call", className: "border-sky-400/30 bg-sky-400/10 text-sky-300" },
  easy_no: { text: "Keep raw", className: "border-rose-500/30 bg-rose-500/10 text-rose-300" },
...
function SweepRow({ entry }: { entry: GradingDeskEntry }) {
  ...
  const badge = VERDICT_BADGE[desk.verdict];
  ...
  const bar =
    desk.verdict === "judgment_call" && desk.ten_confidence_needed != null
      ? `${Math.round(desk.ten_confidence_needed * 100)}%`
      : desk.verdict === "easy_yes"
        ? "any"
        : "—";
  ...
  return (
    <tr>
      <td className="px-4 py-3">
        ...
        {desk.thin_data ? (
          <span
            ...
            title="Thin data — this verdict rests on very few sales."
          >
            !
          </span>
        ) : null}
      ...
      <td className="px-4 py-3 text-sky-200">{bar}</td>
